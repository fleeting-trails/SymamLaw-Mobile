...
function ExamsArchived() {
  const { navigate } = useAppNavigation();
  const dispatch = useAppDispatch();
  const examState = useAppSelector((state) => state.exam);
  const exams = examState.archivedExams.data;
  const loading = examState.loading.fetchArchivedExams;
  const loadMore = examState.loading.fetchArchivedExamsMore;
  const theme = useAppTheme();
  useEffect(() => {
    handleFetchArchivedExams();
  }, []);

  const handleFetchArchivedExams = async () => {
    try {
      dispatch(fetchArchivedExams()).unwrap();
    } catch (error) {
  ...
  const handleLoadMore = () => {
      try {
        dispatch(fetchArchivedExamsMore());
      } catch (error) {
    ...
    <ScreenLoading isLoading={loading}>
      ...
      >
        <View className="flex flex-row flex-wrap gap-2 py-5 border-b-slate-300 border-b-[1px]">
          <CustomText variant="300i">Archived Exams</CustomText>
        </View>
        <FlatList
          data={exams}
          renderItem={({ item }) => (
